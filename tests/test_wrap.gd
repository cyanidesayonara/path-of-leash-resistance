...
func _initialize() -> void:
	...
	_check(leash.winding() > 1.0, "spiral should register winding turns (got %.2f)" % leash.winding())
	...
	_check(leash.winding() > 0.8, "tightening should keep the coil (winding %.2f)" % leash.winding())
	...
	_check(leash.winding() < 0.5, "fully pulled away means unwound (winding %.2f)" % leash.winding())
